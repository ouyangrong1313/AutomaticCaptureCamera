...
#define BackImageName @"navigationBackground.png"
#pragma mark -

#pragma mark - /*** 相册浏览调用 ***/
@implementation ZYQAssetVCHelp
/**
...
#pragma mark - 提醒用户选择超出最大数量
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"最多可上传%ld张图片",(long)_maxNumberSelect] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] ;
            [alert show];
            return;
        }
    }
...
#pragma mark - 重写set方法
- (void)setMaxNumberSelect:(NSInteger)maxNumberSelect{
    _maxNumberSelect = maxNumberSelect ;
    if (_tapAssetView) {
        _tapAssetView.maxNumberSelect = _maxNumberSelect ;
    }
}
...
#pragma mark - 把最大的选择数量 传到下个页面
                assetView.maxNumberSelect = _maxNumberSelect;
...
- (void)setupButtons
{
    SSCustomNarBarButton *backButton = [SSCustomNarBarButton buttonWithType:UIButtonTypeCustom] ;
    backButton.frame = CGRectMake(0, 0, 50, 40);
    //backButton.titleLabel.font = [UIFont systemFontOfSize:17];
    [backButton setTitle:@"相簿" forState:UIControlStateNormal];
    backButton.titleLabel.font = [UIFont systemFontOfSize:14];
    [backButton setTitleColor:UIColorFromHex(0x666666) forState:UIControlStateNormal];
    [backButton setImage:[UIImage imageNamed:@"backBarButtonItemImage"] forState:UIControlStateNormal];
    [backButton addTarget:self action:@selector(backPicking) forControlEvents:UIControlEventTouchUpInside];
    ...
    //----修改完成按钮为白色看不清楚的问题(导航栏也是白色)  fixed by leixiang
    SSCustomNarBarButton *rightBtn = [SSCustomNarBarButton buttonWithType:UIButtonTypeCustom] ;
    [rightBtn setTitle:@"完成" forState:(UIControlStateNormal)];
    rightBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    rightBtn.frame = CGRectMake(0, 0, 50, 40);
    [rightBtn setTitleColor:UIColorFromHex(0x666666) forState:UIControlStateNormal];
    [rightBtn addTarget:self action:@selector(finishPickingAssets:) forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:rightBtn];

//    [UIBarButtonItem addBarButtonToRight:[UIBarButtonItem barButtonItemNoBordWithTitle:NSLocalizedString(@"完成", nil) target:self action:@selector(finishPickingAssets:)] viewController:self width:-5];
...
//    self.navigationItem.leftBarButtonItem.tintColor = [UIColor whiteColor];
        ...
        //        statusBarView.backgroundColor=[UIColor stringToColor:@"#e51c24"];
        
        statusBarView.backgroundColor = [UIColor colorWithPatternImage:backImage];
        [self.navigationController.navigationBar addSubview:statusBarView];
        //    设置导航栏背景图片
        [self.navigationController.navigationBar setBackgroundImage:backImage forBarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
    }
    else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.delegate=self;
#pragma mark - 把最大的选择数量 传到下个页面
    cell.maxNumberSelect = picker.maximumNumberOfSelection ;
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (self.number < indexPaths.count) {
    ...
    self.title = [NSString stringWithFormat:@"已选中%lu张图片",(unsigned long)indexPaths.count];
}
...
- (void)setupButtons
{
    ...
    if (picker.showCancelButton) {
        ...
        [UIBarButtonItem addBarButtonToRight:[UIBarButtonItem barButtonItemNoBordWithTitle:@"取消" target:self action:@selector(dismiss:)] viewController:self width:-5];
    }
}
...
#pragma mark - 把最大的选择数量 传到下个页面
    vc.maxNumberSelect = _maxNumberSelect ;
