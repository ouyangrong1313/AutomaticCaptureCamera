...
- (void)initView {
    ...
    CGFloat imageWidthMax = kFullScreenWidth;
    CGFloat imageHeightMax = kFullScreenHeight;
    ...
    _screenRatio = kFullScreenHeight / kFullScreenWidth; //屏幕高宽比
    CGRect imageViewFrame = CGRectZero;
    if (_imageRatio > _screenRatio) { //如果图片高宽比更大,以图片高度登录屏幕高度
    ...
    self.overlayView.backgroundColor = UIColorFromHexA(COLOR_000000, 0.3);
...
//    [self.view addSubview:effectView];
    ...
    self.ratioView.autoresizingMask = UIViewAutoresizingNone;
    UIImageView *img = [[UIImageView alloc] initWithFrame:self.ratioView.bounds];
    img.image = [UIImage imageNamed:@"图片选择框"];
    [self.ratioView addSubview:img];
...
-(UIImage *)getSubImage{
    ...
    SSLog(@"图片的尺寸:%@",NSStringFromCGSize(smallImage.size));
    return smallImage;
}
